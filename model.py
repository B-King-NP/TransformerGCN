...
class spatialGCN(nn.Module):
    def __init__(self, adj, in_channel, out_channel):
        ...
        self.Theta = nn.Linear(in_channel, out_channel)
...
class Spatial_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        score = torch.matmul(x, x.transpose(1, 2)) / math.sqrt(in_channels)  # (b*t, N, N)
...
class spatialAttentionGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        batch_size, num_of_vertices, num_of_timesteps, in_channels = x.shape
        spatial_attention = self.SAt(x) / math.sqrt(in_channels)  # (batch, T, N, N)
        x = x.permute(0, 2, 1, 3).reshape((-1, num_of_vertices, in_channels))  # (b*t,n,f_in)
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    if dropout is not None:
        p_attn = dropout(p_attn)  # p_attn: (batch, N, h, T1, T2)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key, value = [layer(x).view(nbatches, N, -1, self.h, self.d_k).transpose(2, 3)
...
class MultiHeadAttentionAwareTemporalContex_qc_kc(nn.Module):  # key causal; query causal;
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key = [l(x.permute(0, 3, 1, 2))[:, :, :, :-self.padding].contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for
...
class MultiHeadAttentionAwareTemporalContex_q1d_k1d(nn.Module):  # 1d conv on query, 1d conv on key
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # deal with key and query: temporal conv
        # (batch, N, T, d_model)->permute(0, 3, 1, 2)->(batch, d_model, N, T) -conv->(batch, d_model, N, T)-view->(batch, h, d_k, N, T)-permute(0,3,1,4,2)->(batch, N, h, T, d_k)
        query, key = [l(x.permute(0, 3, 1, 2)).contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for
                      l, x in zip(self.conv1Ds_aware_temporal_context, (query, key))]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory):
        ...
        if self.residual_connection or self.use_LayerNorm:
            x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))  # output: (batch, N, T', d_model)
            x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m))  # output: (batch, N, T', d_model)
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_for_predict,
               dropout=.0, kernel_size=3, smooth_layer_num=0, residual_connection=True, use_LayerNorm=True):
    c = copy.deepcopy
    ...
    position_wise_gcn = PositionWiseGCNFeedForward(spatialAttentionGCN(norm_Adj_matrix, d_model, d_model),
                                                   dropout=dropout)
    trg_dense = nn.Linear(decoder_output_size, d_model)

    attn_ss = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, kernel_size, dropout=dropout)  # encoder's trend-aware attention is conv1d
    attn_st = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, kernel_size, dropout=dropout)
    att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, kernel_size, dropout=dropout)  # decoder's trend-aware attention is causal conv

    encode_temporal_position = TemporalPositionalEncoding(d_model, dropout, max_len)  # decoder temporal position embedding
